...
@Component({
  selector: 'app-informacion-pr',
  templateUrl: './informacion-pr.page.html',
  styleUrls: ['./informacion-pr.page.scss'],
  standalone: false,
})
export class InformacionPRPage implements OnInit {
...
PRU01CODI: number = 0;
  ...
  async ngOnInit() {
    try {
      ...
      this.PRU01CODI = parseInt(idFromUrl, 10);

      if (isNaN(this.PRU01CODI)) {
  ...
  async cargarRegistros() {
    try {
      ...
      const registrosFiltrados = registrosLecturas.filter(registro => registro.PRU01CODI === this.PRU01CODI);
  ...
  async guardarCambios() {
    try {
      for (const registro of this.registros) {
        await this.ionicStorageService.guardarOActualizarPredioRur(registro);
      }
  ...
  mostrarOpciones() {
    ...
    accion(tipo: string) {
      if (tipo === 'agregar') {
        ...
        this.router.navigate(['/contruccion-r/'], { queryParams: { PRU01CODI: this.PRU01CODI } });
      } else if (tipo === 'actualizar') {
        // Para actualizar, pasa el valor de PUR01CODI como parámetro
        this.router.navigate([`/contruccion-r/${this.PRU01CODI}`]);
      }
